...
def evaluate_fitness(particle, sensor_positions, sensor_types, t_prev, current_time,
    ...
    total_util, new_t_prev, _ = compute_total_utility(
        particle.position, sensor_positions, sensor_types,
        current_time, t_prev,
        R, Vmax, Vmin, G_St, G_Ur, lambda_S, P_st, P_U_min_r, h,
        P_tx_uav, G_tx_uav, G_rx_uav, SINR_threshold, N, cm, km, Q, N0
    )
    return total_util, new_t_prev
...
def run_pso(uav_init_positions, sensor_positions, sensor_types, t_prev, current_time, current_utility,
    ...
    for iteration in range(max_iterations):
        ...
        for p in swarm:
            # Tính tiện ích
            fitness, new_t_prev = evaluate_fitness(p, sensor_positions, sensor_types, t_prev, current_time,
                                                   R, Vmax, Vmin, G_St, G_Ur, lambda_S, P_st, P_U_min_r, h,
                                                   P_tx_uav, G_tx_uav, G_rx_uav, SINR_threshold, N, cm, km, Q, N0)
            p.fitness = fitness
    ...
    return global_best_fitness, global_best_position, best_t_prev, better_than_current
...
def main():
    ...
    max_pso_iterations = 100  # Số vòng lặp PSO tối đa
    num_time_steps = 50  # Số time step mô phỏng
    convergence_threshold = 0.0001  # Ngưỡng hội tụ (epsilon)
    omega = 0.8  # Hệ số quán tính
    c1 = 1.4  # cognitive
    c2 = 1.4  # social
    speed_factor = 1.2  # Nhân thêm speed_factor khi cập nhật vận tốc
    ...
    for step in range(1, num_time_steps + 1):
        ...
        best_fitness, best_positions, best_t_prev, is_better = run_pso(
            current_uav_positions, sensor_positions, sensor_types,
        ...
        if is_better and best_positions is not None:
            ...
            t_prev = best_t_prev  # Cập nhật t_prev dựa trên đánh giá PSO

            # 4. Thu thập cảm biến ở vị trí mới
            t_prev, collected_sensors = update_sensors_after_collection(
                current_uav_positions, sensor_positions, t_prev, current_time,
                G_St, G_Ur, lambda_S, P_st, P_U_min_r, h
            )
