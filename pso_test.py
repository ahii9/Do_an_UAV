...
def run_pso(uav_init_positions, sensor_positions, sensor_types, t_prev, current_time, current_utility,
    ...
    prev_global_best = -math.inf  # Khởi tạo prev_global_best
    iterations = 0
    id=0
    ...
    for iteration in range(max_iterations):
        ...
        # Kiểm tra hội tụ dựa trên global_best_fitness, chỉ sau ít nhất 10 vòng
        if abs(global_best_fitness - prev_global_best) < convergence_threshold:
            id=id+1
        if id>20:
            print(f"PSO converged after {iterations-20} iterations")
            break
        prev_global_best = global_best_fitness  # Cập nhật prev_global_best

    better_than_current = global_best_fitness > current_utility
    if better_than_current:
        print(f"Found better position: {global_best_fitness:.4f} > {current_utility:.4f}")
    else:
        print(f"No better position found: {global_best_fitness:.4f} <= {current_utility:.4f}")
...
def main():
    ...
    convergence_threshold = 0.001 # Ngưỡng hội tụ (epsilon)
    ...
    for step in range(1, num_time_steps + 1):
        ...
        best_fitness, best_positions, is_better,id = run_pso(
            current_uav_positions, sensor_positions, sensor_types,
